...
Func guidedFilter(Func I, Func p, int r, float epsilon) {
    ...
    pair2int.compute_root();
    a_m.compute_at(q, Var::outermost());
    b_m.compute_at(q, Var::outermost());
    a.compute_at(q, Var::outermost());
    b.compute_at(q, Var::outermost());
...
Func stereoGF_scheduled(Func left, Func right, int width, int height, int r, float epsilon, int numDisparities, float alpha, float threshColor, float threshGrad)
{
    ...
    filtered_left.compute_at(disp_left_right, rd).vectorize(x, vector_width);
    filtered_right.compute_at(disp_left_right, rd).vectorize(x, vector_width);
...
//             }
//         }
//         output3.compile_to_lowered_stmt("mean.html", {}, HTML);
...
int main(int argc, char ** argv)
{
    ...
    ImageParam input_0(type_of<float>(), 3);
    ImageParam input_1(type_of<float>(), 3);
    int width = im0.width(), height = im0.height();
    Func left("left"), right("right");
    left(x,y,c) = input_0(clamp(x, 0, width-1), clamp(y, 0, height-1), c);
    right(x, y, c) = input_1(clamp(x, 0, width-1), clamp(y, 0, height-1), c);

    Func disp = stereoGF_scheduled(left, right, im0.width(), im0.height(), 9, 0.0001, numDisparities, 0.9, 0.0028, 0.008);

    std::vector<Argument> args = {input_0, input_1};
    disp.compile_to_file("guidedFilter", args, get_target_from_environment());
